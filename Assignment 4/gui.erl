...
%%    helper:start(to_atom(ClientName), client:initial_state(Nickname, GUIName) , fun client:main/1),
    ...
    Frame = wxFrame:new(Server, -1, "Chat",[{pos, {200,200}},{size,{500,500}}]),
    Parent = Frame,
...
hash(ID) ->
    erlang:phash2(ID,10000).

focus(ID) ->
    W = wxWindow:findWindowById(hash(ID)),
    wxWindow:setFocus(W).

typed_search(ID, no_cast) ->
    Result = wxWindow:findWindowById(hash(ID)),
    Result ;
...
label(ClientID, ID, Widget) ->
    ...
    _Result = wxWindow:setId(Widget, hash(Label)),
    ok.
...
chatroom_id(ChatroomName) ->
    S = lists:foldl(fun(S, Acc) -> io_lib:format("~p", [S])++Acc end, "", lists:sublist(ChatroomName,15)),
